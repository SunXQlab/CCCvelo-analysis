...
for (i in files){
  ...
  # modified the shape of LRTF_para
  new_MRR1 <- matrix(0, nrow=10, ncol=3)
  new_MRR1[1,1] <- LRTF_para[1,1];new_MRR1[5,1] <- LRTF_para[1,1];new_MRR1[9,1] <- LRTF_para[1,1];
  new_MRR1[1,2] <- LRTF_para[1,2];new_MRR1[5,2] <- LRTF_para[1,2];new_MRR1[8,2] <- LRTF_para[2,2];new_MRR1[9,2] <- LRTF_para[1,2];
  new_MRR1[6,3] <- LRTF_para[2,3];new_MRR1[8,3] <- LRTF_para[2,3];
  
  new_MM1 <- matrix(0, nrow=11, ncol=3)
  new_MM1[1,1] <- LRTF_para[3,1];new_MM1[5,1] <- LRTF_para[3,1];new_MM1[9,1] <- LRTF_para[3,1];
  new_MM1[1,2] <- LRTF_para[3,2];new_MM1[5,2] <- LRTF_para[3,2];new_MM1[8,2] <- LRTF_para[4,2];new_MM1[9,2] <- LRTF_para[3,2];
  new_MM1[6,3] <- LRTF_para[4,3];new_MM1[8,3] <- LRTF_para[4,3];
  new_MM1[11,1] <- LRTF_para[5,1];new_MM1[11,2] <- LRTF_para[5,2];new_MM1[11,3] <- LRTF_para[5,3];
  new_para1 <- rbind(new_MRR1,new_MM1)
  
  distMat <- as.matrix(dist(locaMat))
  diag(distMat) <- 1
  
  # save result for python input
  f_path <- paste0(getwd(),"/newInput/fan_modified_initial_y0/")
  ...
  write.table(TFTG_link,file=paste0(output_fpath, '/TFTG_links.csv'),
              sep = ",",row.names = FALSE,col.names = TRUE)
  write.table(new_para1,file=paste0(output_fpath, '/LRTF_paras.csv'),
              sep = ",",row.names = FALSE,col.names = TRUE)
